...
class DriverBehaviorClassifier:
    ...
    def generate_labels(self, features):
        ...
        if 'surround_min_dist' in features.columns:
            # Low distance to other vehicles increases aggression score
            aggression_score += normalize(1 / (features['surround_min_dist'] + 1)) * 0.15
...
class DriverBehaviourClassifierVersionThree:
    """
    Generates features, pseudo-labels, and trains a Random Forest while preventing label leakage.
    """
    def engineer_features(self, df, window_size=50):
        """Engineer rolling-window features from raw sensor data"""
        features = pd.DataFrame()
        features['time'] = df['time']
        ...
        features['jerk_long_max'] = df['oveBodyJerkLongitudinalX'].rolling(window_size, min_periods=1).max().abs()
        features['jerk_lat_std'] = df['oveBodyJerkLateralY'].rolling(window_size, min_periods=1).std()
        features['speed_std'] = np.sqrt(df['oveBodyVelocityX']**2 + df['oveBodyVelocityY']**2).rolling(window_size, min_periods=1).std()
        features['steering_angle_std'] = df['steeringWheelAngle'].rolling(window_size, min_periods=1).std()
        features['steering_rate'] = df['steeringWheelAngle'].diff().abs().rolling(window_size, min_periods=1).mean()
        features['throttle_changes'] = df['throttle'].diff().abs().rolling(window_size, min_periods=1).sum()
        ...
        if 'short_headway_ratio' in df.columns:
            features['short_headway_ratio'] = df['aheadTHW'].rolling(window_size, min_periods=1).apply(
                lambda x: (((x > 0) & (x < 2)).sum() / len(x)) if len(x) > 0 else 0
            )
        ...
        # --- INATTENTIVE DRIVING ---
        features['lateral_pos_std'] = df['ovePositionLateralR'].rolling(window_size, min_periods=1).std()
        if 'ndrt_error_rate' in df.columns:
            features['ndrt_error_rate'] = (df['arrowsWrongCount'] + df['arrowsTimeoutCount']).rolling(window_size, min_periods=1).mean()
        if 'gaze_off_road_ratio' in df.columns:
            features['gaze_off_road_ratio'] = ((df['openxrGazeHeading'].abs() > 30) | (df['openxrGazePitch'].abs() > 20)).rolling(window_size, min_periods=1).mean()
        if 'eyelid_low_ratio' in df.columns:
            features['eyelid_low_ratio'] = (df['varjoEyelidOpening'] < 0.3).rolling(window_size, min_periods=1).mean()
        if 'rrInterval' in df.columns:
            diff_rr = df['rrInterval'].diff()
            features['hrv_rmssd'] = (diff_rr ** 2).rolling(window_size, min_periods=1).mean() ** 0.5

        # Control smoothness (used for labels)
        features['control_smoothness'] = 1 / (1 + features.get('steering_rate', 0) + features.get('throttle_changes', 0) / 10)
    ...
    def generate_labels(self, features):
        """Generate heuristic labels using only a subset of features"""
        label_features = [
            'accel_long_std', 'jerk_long_max', 'jerk_lat_std', 
            'speed_std', 'steering_angle_std', 'steering_rate', 'throttle_changes',
            'hr_mean', 'hr_change', 'short_headway_ratio',
            'lateral_pos_std', 'ndrt_error_rate', 'gaze_off_road_ratio', 'eyelid_low_ratio', 'hrv_rmssd', 'control_smoothness'
        ]
        self.label_features = [f for f in label_features if f in features.columns]
        ...
        # Aggression score
        aggression_score = sum(normalize(features[f]) for f in ['accel_long_std','jerk_long_max','jerk_lat_std',
                                                                'speed_std','steering_angle_std','steering_rate',
                                                                'throttle_changes'] if f in features.columns)
        if 'hr_mean' in features.columns:
            aggression_score += normalize(features['hr_mean']) + normalize(features['hr_change'])
        if 'short_headway_ratio' in features.columns:
            aggression_score += normalize(features['short_headway_ratio'])

        # Inattention score
        inattention_score = sum(normalize(features[f]) for f in ['lateral_pos_std','ndrt_error_rate','gaze_off_road_ratio',
                                                                 'eyelid_low_ratio','hrv_rmssd','control_smoothness'] if f in features.columns)
        inattention_score += normalize(1 - features.get('control_smoothness', 0))

        # Thresholds
        agg_thresh = np.percentile(aggression_score, 70)
        inatt_thresh = np.percentile(inattention_score, 70)

        labels = np.zeros(len(features), dtype=int)  # Attentive by default
        labels[(aggression_score > agg_thresh) & (inattention_score <= inatt_thresh)] = 2
        labels[(inattention_score > inatt_thresh) & (aggression_score <= agg_thresh)] = 1
        labels[(aggression_score > agg_thresh) & (inattention_score > inatt_thresh)] = 2
