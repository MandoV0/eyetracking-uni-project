...
class DriverStateClassifier:
    ...
    def prepare_features_for_training(self, features):
        exclude_cols = ["row_id", "time", "aggression_score", "inattention_score", "session_id"]
        feature_cols = [c for c in features.columns if c not in exclude_cols]
        if self.feature_cols is None:
            self.feature_cols = feature_cols
        return features[self.feature_cols].fillna(0)
    ...
    def train_with_cross_validation(self, all_features, all_labels, session_ids):
        ...
        print(f"Mean Train-Test Gap: {np.mean(gaps):+.4f}")
        ...
        cm = confusion_matrix(cv_true_labels, cv_predictions, labels=[0, 1, 2])
    ...
    def train_final_model(self, features, labels):
        ...
        return accuracy_score(labels, train_pred)
    ...
    def save_artifacts(self, out_path):
        payload = {
            "model": self.model,
            "scaler": self.scaler,
